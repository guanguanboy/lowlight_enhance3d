...
def val_without_gan(val_loader, model, epoch, device):
...
def train(save_model=False):
    ...
    checkpoint_path = 'checkpoints/withoutgan'
    ...
    for epoch in range(n_epochs):
        ...
        for condition, real in tqdm(train_dataloader):
            ...
            gen_loss = recon_criterion(fake, real)
        ...
        val_without_gan(val_dataloader, gen, epoch, DEVICE)

        if save_model:
            torch.save({
                'gen': gen.state_dict(),
                'gen_opt': gen_opt.state_dict(),
            }, f"{checkpoint_path}/pix2pix3d_{epoch}.pth")
